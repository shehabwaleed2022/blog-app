<x-layout :pageTitle="$pageTitle">
    <section class="px-6 py-8"> 
      ...
      <div class="container mx-auto max-w-sm px-4 py-8">
        ...
        <form method="POST" action="{{route('post.create.store')}}" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          @csrf
          <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2" for="post-title">Post Title</label>
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              id="post-title" type="text" name="title" placeholder="Enter the title of your post" value="{{old('title')}}">
            @error('title')
