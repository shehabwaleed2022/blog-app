<header class="max-w-xl mx-auto mt-20 text-center">
    ...
    <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-4">
          ...
          <div x-data="{ show: false }">
            <button @click="show = ! show" @click.away="show = false" class="py-2 pl-3 pr-9 text-sm font-semibold w-full lg:w-32 text-left flex lg:inline-flex">
              ...
              @foreach ($categories as $category)
                <a href="/categories/{{$category->name}}" class="block text-left px-3 text-sm leading-5 hover:bg-blue-300 focus:bg-blue-300 hover:text-white focus:text-white {{ isset($currentCategory) && $currentCategory->is($category) ? 'bg-blue-500 text-white' : '' }}">{{ucwords($category->name)}}</a>
              @endforeach
              </div>
        ...
        <div class="relative flex lg:inline-flex items-center bg-gray-100 rounded-xl">
            ...
            <svg class="transform -rotate-90 absolute dinter-events-none" style="right: 12px;" width="22" height="22" viewBox="0 0 22 22">
                <g fill="none" fill-rule="evenodd">
                    <path stroke="#000" stroke-opacity=".012" stroke-width=".5" d="M21 1v20.16H.84V1z"></path>
                    <path fill="#222" d="M13.854 7.224l-3.847 3.856 3.847 3.856-1.184 1.184-5.04-5.04 5.04-5.04z"></path>
                </g>
